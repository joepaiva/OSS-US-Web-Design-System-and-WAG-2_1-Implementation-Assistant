...
# SD-Agile spec-first method — Claude Code instructions

This repository ships a **Builder Package** (the authoritative spec) and a chassis. Read `REQUIREMENTS.md` (what) · `DESIGN.md` (how) · `TASKS.md` (build order) · `TEST-SCENARIOS.md` (TS-NNN tests) before writing code for a task not covered below.

## CONSTITUTION.md — supreme, inlined so every session sees it immediately
...
## Rules
1. **Constitution is supreme** — if anything conflicts with it, the constitution above wins.
2. **Spec-first** — code derives from the spec; never let code drift from it. If you find a genuine minor spec gap, name it explicitly rather than silently improvising.
3. **Tests are requirements** — never weaken/skip a test to pass; fix the code. Every TS-NNN needs a passing test, cited by ID in the test itself.
4. **The chassis is immutable** — implement only in the slots (`app/slots/**`); never modify chassis core (`app/**`).
5. **Verify before done** — build + the chassis test suite must pass; for anything with a browser surface, a Playwright end-to-end test that asserts the rendered result is required.

Use `.claude/commands/next-task.md` to work through the approved task list one task at a time. A Compliance Corridor scan checks this repository's compliance with the Constitution and the declared security profile independently of this file — write code that will pass it, not just this prompt.
